...
使用独立的 `internal/search` 模块实现 `POST /api/v1/secret/search`，只匹配 Key 和备注，不修改现有 Secret 增删改、list 与历史查询
...
- 短词或没有连续三个字母、数字的模式仅允许限定项目或目录查询，例如 `ab`、`密码`、`a_b`；中文按 Unicode 字符判断，不按字节数
- 空关键词表示范围内分页浏览，不添加文本匹配条件
- 每页默认 20 组，上限 200，复用 page.Request 和 page.Response；total 按 groupId 去重统计，响应只有 total 和 list
- 标签本阶段只展示，不提供标签筛选；历史在点击后加载，不随搜索响应预加载
- 未支持的字段会报参数错误，包括 tagIdList，避免调用者误以为筛选已经生效
...
第一阶段沿用共享密钥接口的认证边界，资源和环境授权仍待统一权限模型落地，不另建临时权限体系
当前通过认证的用户可查询共享密钥，`applyPermissionFilter` 是明确的待办，不应将本阶段表述为已实现资源授权
后续权限必须同时作用于 count、分页 groupId 与环境详情三个查询，过滤结果为空不能被解释为不限制
...
- SQL 逻辑中范围与关键词同时生效，由优化器决定扫描顺序，不强制先物化全库候选集
